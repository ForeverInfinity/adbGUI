...
namespace adbGUI.Forms
{
    public partial class InstallUninstall : ExtForm
    {
        ...
        private void Btn_InstallUninstallInstall_Click(object sender, EventArgs e)
        {
            ...
            {
                int sdkVersion = Convert.ToInt32(HelperClass.ExecuteWithOutput("adb", "shell getprop ro.build.version.sdk"));
                string command = "adb install ";
                if (sdkVersion >= 34)
                    command += "--bypass-low-target-sdk-block ";
                HelperClass.Execute(command + s);
            }
            else
            {
